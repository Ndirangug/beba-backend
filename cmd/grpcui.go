...
func Grpcui() {
	...
	if err != nil {
		logger.Panic("failed to create client to local server for use with grpcui: %v", err)
	}
	...
	if err != nil {
		logger.Panic("failed to create client to local server: %v", err)
	}
	...
	if err := http.ListenAndServe(fmt.Sprintf(":%s", port), serveMux); err != nil {
		logger.Panic("Failed to start http listener for grpcui. Error %v", err)
	}
